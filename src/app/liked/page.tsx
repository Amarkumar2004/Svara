...
import { getGuestLikes } from "@/lib/guest";
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="glass rounded-3xl p-6 lg:p-8">
        <div className="flex flex-col lg:flex-row items-center gap-6">
          <div className="w-40 h-40 lg:w-52 lg:h-52 rounded-2xl bg-gradient-to-br from-[#e17055] to-pink-400 flex items-center justify-center flex-shrink-0 shadow-2xl">
            <Heart className="w-16 h-16 text-white" />
          </div>
          <div className="text-center lg:text-left">
            <p className="text-xs uppercase tracking-[0.15em] text-muted font-semibold mb-2">Saved Locally</p>
            <h1 className="text-3xl lg:text-5xl font-extrabold mb-2">Liked Songs</h1>
            <p className="text-muted mb-6">{songs.length} liked song{songs.length !== 1 ? "s" : ""}</p>
            {songs.length > 0 && (
      ...
      {songs.length === 0 ? (
        <div className="text-center py-20 animate-fade-in">
          <div className="w-20 h-20 rounded-2xl bg-white/30 dark:bg-white/5 flex items-center justify-center mx-auto mb-5">
            <Heart className="w-10 h-10 text-muted/50" />
          </div>
          <h3 className="text-xl font-bold mb-1">No liked songs yet</h3>
          <p className="text-sm text-muted mb-6">Heart songs you love to save them here</p>
